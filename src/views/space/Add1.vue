...
export default {
  ...
  methods: {
    ...
    handleDetailAdd() {
      let _this = this
      let j = { id: null, name: '', number: '', imgMain: '' }
    ...
    handleDialogYes(e) {
      ...
      console.log('商品弹窗确定--选中商品', e)
      var obj = {}
      this.detail.title.forEach(function(item, index) {
        if(item.field == 'imgMain' && e[0][item.field]) {
          obj[item.field] = imageHttpsUrlPTF()+e[0][item.field]
        }else {
          obj[item.field] = e[0][item.field]
        }
      })

      let data = JSON.parse(JSON.stringify(obj));
      for (let k in data) this.detail.tableData[this.detailRowIndex][k] = data[k];
    },
    // 商品弹窗取消
    handleDialogNo(type) {
      this.$message('已取消')
